...
def northwest_corner(supply, demand, costs=None):
    """
    Résolution de problème de transport par la méthode du coin Nord-Ouest
    
    Args:
        supply (list): Offres des sources
        demand (list): Demandes des destinations
        costs (np.array): Matrice des coûts (optionnelle)
    
    Returns:
        tuple: (statut, message, matrice d'allocation, coût total)
    """
    try:
